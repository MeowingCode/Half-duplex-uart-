...
public:
    using UARTByteTRIG = UARTByteTRIGTypeDef;
    ...
    bool isConnected() { return connection; }
    uint8_t hasCommand() { return (state == STATE_WAIT) ? rx_command : 0; }

protected:
    void reset()
    {
        tx_command = 0; tx_length = 0; tx_buffer = nullptr;
        rx_command = 0; rx_length = 0; rx_buffer = nullptr;
        echo_count = 0;
    }
    ...
    void commandTransmit() { THR() = (tx_command > 0) ? tx_command : HELLO; }
    ...
    void commandReceived() { rx_command = RBR(); }
    ...
    {
        FCR() = (2 << 6) | RB_FCR_TX_FIFO_CLR | RB_FCR_RX_FIFO_CLR | RB_FCR_FIFO_EN;
        LCR() = RB_LCR_WORD_SZ;
        IER() = RB_IER_TXD_EN;
        DIV() = 1;
    }
    ...
    {
        uint32_t x = (10 * GetSysClock() / 8 / baudrate + 5) / 10;
        DL() = (uint16_t)x;
    }
...
class UartMasterPort
    : public UartPort<BASE, TX_PIN, RX_PIN, UartMasterPort<BASE, TX_PIN, RX_PIN>>
{
...
private:
    void proceed()
    {
        if (this->tx_length) { this->setTxDataState(); this->dataTransmit(); }
        else if (this->rx_length) { this->setTxCommandState(); this->commandTransmit(); }
        else setIdleState();
    }
